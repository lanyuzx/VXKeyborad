...
#import "LLInputBar.h"
#import "LLRecordView.h"
#import "LLHelper.h"
#import "LLUIKit.h"
#import <AVFoundation/AVFoundation.h>

@interface LLInputBar() <UITextViewDelegate, AVAudioRecorderDelegate,LLResponderTextViewDelegate>
@property (nonatomic, strong) LLRecordView *record;
@property (nonatomic, strong) NSDate *recordStartTime;
@property (nonatomic, strong) AVAudioRecorder *recorder;
@property (nonatomic, strong) NSTimer *recordTimer;
@end

@implementation LLInputBar
...
- (void)setupViews
{
    ...
    _inputTextView = [[LLResponderTextView alloc] init];
    _inputTextView.placeHolder = @"说点什么吧";
    _inputTextView.delegate = self;
    _inputTextView.responderDelegate = self;
...
- (void)recordBtnDown:(UIButton *)sender
{
    ...
    if(permission == AVAudioSessionRecordPermissionGranted){
        if(!_record){
            _record = [[LLRecordView alloc] init];
...
- (void)backDelete
{
    ...
    NSString *handleText;
    NSString *appendText;
    if (range.location == _inputTextView.text.length) {
        handleText = _inputTextView.text;
        appendText = @"";
    }else {
        handleText = [_inputTextView.text substringToIndex:range.location];
        appendText = [_inputTextView.text substringFromIndex:range.location];
    }
...
- (void)deleteBackward:(NSString *)text appendText:(NSString *)appendText
{
    if (IsTextContainFace(text)) { // 如果最后一个是表情
        ...
        if (text.length >= 2) {
            ...
            if ([LLHelper isEmoji:tempString]) { // 如果是Emoji表情
...
- (void)startRecord
{
    ...
    NSString *path = [TUIKit_Voice_Path stringByAppendingString:[LLHelper genVoiceName:nil withExtension:@"m4a"]];
