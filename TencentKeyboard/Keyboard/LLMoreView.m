...
@interface LLMoreView () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
- (void)setupViews
{
    self.backgroundColor = TMoreView_Background_Color;
    ...
    [_moreCollectionView registerClass:[LLInputMoreCell class] forCellWithReuseIdentifier:TMoreCell_ReuseId];
    ...
    _lineView.backgroundColor = TMoreView_Line_Color;
    [self addSubview:_lineView];

    _pageControl = [[UIPageControl alloc] init];
    _pageControl.currentPageIndicatorTintColor = [UIColor grayColor];
    _pageControl.pageIndicatorTintColor = TMoreView_Page_Color;
    [self addSubview:_pageControl];
}

- (void)defaultLayout
{
    CGSize cellSize = [LLInputMoreCell getSize];
    CGFloat collectionHeight = cellSize.height * _rowCount + TMoreView_Margin * (_rowCount - 1);

    _lineView.frame = CGRectMake(0, 0, self.frame.size.width, TMoreView_Line_Height);
...
{
    LLInputMoreCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:TMoreCell_ReuseId forIndexPath:indexPath];
    LLInputMoreCellData *data;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(_delegate && [_delegate respondsToSelector:@selector(moreView:didSelectMoreCell:)]){
        if ([cell isKindOfClass:[LLInputMoreCell class]]) {
            [_delegate moreView:self didSelectMoreCell:(LLInputMoreCell *)cell];;
        }
    }
}
...
{
    return [LLInputMoreCell getSize];
}
