...
#import "LLResponderTextView.h"
#import "THeader.h"
@implementation LLResponderTextView
...
- (void)deleteBackward
{
    if (IsTextContainFace(self.text)) { // 如果text中有表情
        if ([self.delegate respondsToSelector:@selector(textViewDeleteBackward:)]) {
            [self.responderDelegate textViewDeleteBackward:self];
        }
    }else {
...
- (NSUInteger)numberOfLinesOfText{
    return [LLResponderTextView numberOfLinesForMessage:self.text];
}
...
+ (NSUInteger)numberOfLinesForMessage:(NSString *)text{
    return (text.length / [LLResponderTextView maxCharactersPerLine]) + 1;
}
