...
@implementation LLFaceGroup
@end


@interface LLFaceView () <UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
...
- (void)setupViews
{
    self.backgroundColor = TFaceView_Background_Color;
    ...
    [_faceCollectionView registerClass:[LLFaceCell class] forCellWithReuseIdentifier:TFaceCell_ReuseId];
    ...
    _lineView.backgroundColor = TFaceView_Line_Color;
    [self addSubview:_lineView];

    _pageControl = [[UIPageControl alloc] init];
    _pageControl.currentPageIndicatorTintColor = [UIColor grayColor];
    _pageControl.pageIndicatorTintColor = TFaceView_Page_Color;
    [self addSubview:_pageControl];
}

- (void)defaultLayout
{
    _lineView.frame = CGRectMake(0, 0, self.frame.size.width, TFaceView_Line_Height);
...
- (void)setData:(NSMutableArray *)data
{
    ...
    for (NSInteger groupIndex = 0; groupIndex < _faceGroups.count; ++groupIndex) {
        LLFaceGroup *group = _faceGroups[groupIndex];
    ...
    for (NSInteger curSection = 0; curSection < _sectionCount; ++curSection) {
        ...
        LLFaceGroup *face = _faceGroups[groupIndex.integerValue];
...
{
    LLFaceCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:TFaceCell_ReuseId forIndexPath:indexPath];
...
{
    int groupIndex = [_groupIndexInSection[indexPath.section] intValue];
    LLFaceGroup *faces = _faceGroups[groupIndex];
