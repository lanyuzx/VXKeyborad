...
+ (id)defaultConfig
{
    ...
    static LLKitConfig *config;
    dispatch_once(&onceToken, ^{
        config = [[LLKitConfig alloc] init];
    });
    return config;
}
...
- (void)defaultFace
{
    ...
    if(emojiFaces.count != 0){
        LLFaceGroup *emojiGroup = [[LLFaceGroup alloc] init];
    ...
    if(ttFaces.count != 0){
        LLFaceGroup *ttGroup = [[LLFaceGroup alloc] init];
...
{
    [[LLImageCache sharedInstance] addResourceToCache:path];
}


- (void)addFaceToCache:(NSString *)path
{
    [[LLImageCache sharedInstance] addFaceToCache:path];
}
